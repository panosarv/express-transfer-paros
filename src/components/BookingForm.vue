      ...
      >
        ...
        <div>
          ...
          <div v-if="pickupMode === 'list'" class="mt-4">
            ...
            <LMap
              :zoom="11"
              :center="currentPickupCoords"
              :options="mapOptions"
              class="mt-4" style="height:250px;width:100%"
            >
              <LTileLayer
              ...
              />
              <LMarker
                v-if="form.pickupLocation"
                :lat-lng="currentPickupCoords"
              >
        ...
        <div>
          ...
          <div v-if="dropoffMode === 'list'" class="mt-4">
            ...
            >
              ...
              />
              <LMarker
                v-if="form.dropoffLocation"
                :lat-lng="currentDropoffCoords"
              >
...
const pins = [
  ...
  { name: 'Monastiri', coords: [37.0970, 25.3000] },
  { name: 'Aerodromio - Alyki', coords: [37.0681, 25.2439] },
  { name: 'Drios - ΧΑ', coords: [37.0040, 25.2237] },
  { name: 'Lefkes', coords: [37.1000, 25.3000] },
  { name: 'Piso Livadi', coords: [37.0280, 25.2360] },
  { name: 'Pounta', coords: [37.0250, 25.2810] },
  { name: 'Krios - cabana', coords: [37.0800, 25.2100] },
...
const price = computed(() => {
  const pu = form.value.pickupLocation;
  const doff = form.value.dropoffLocation;
  const table = priceTables[pu];
  if (!table || !table[doff]) return null;
  const bracket = form.value.people <= 4 ? '1-4' : '5-8';
  let p = table[doff][bracket];
